...
export function MetricCalculator({
  ...
  return (
    <div id={calcId} className="not-prose my-8 rounded-xl border border-[#e5e7eb] bg-white overflow-hidden shadow-sm">
      {/* Header */}
      <div className="px-5 py-4 border-b border-[#e5e7eb]" style={{ borderTopWidth: 3, borderTopColor: color }}>
      ...
      <div className="p-5">
            ...
            <div key={field.name}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  max={field.max}
                  step={field.step ?? 1}
                  className={`w-full h-10 rounded-lg border border-[#e5e7eb] bg-[#f8f9fc] text-sm text-[#1a1a2e] focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-colors ${
        ...
        <div className="rounded-lg p-4" style={{ background: `${color}15` }}>
          <div className="text-xs font-medium text-[#6b7280] mb-1">{result.label}</div>
