    ...
    <div className="relative">
      ...
      <div className="px-3 space-y-3 py-2">
        ...
        <AnimatedAccordion title="Acquisition" defaultOpen>
          <div className="space-y-3">
            <TripleField label="Ad Budget ($)" help="Monthly advertising spend" values={[p1.ad_budget, p2.ad_budget, p3.ad_budget]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { ad_budget: v })} onChangeAll={(v) => setPhaseAll({ ad_budget: v })} min={0} step={500} />
            <TripleField label="CPI ($)" help="Cost Per Install — average cost per new app install. Industry: $1–5." values={[p1.cpi, p2.cpi, p3.cpi]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { cpi: v })} onChangeAll={(v) => setPhaseAll({ cpi: v })} min={0.01} step={0.5} />
            ...
            <div className="pt-1 text-[11px] font-medium text-muted-foreground">Organic</div>
            <TripleField label="Organic Spend ($)" help="Monthly spend on organic channels" values={[p1.organic_spend, p2.organic_spend, p3.organic_spend]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { organic_spend: v })} onChangeAll={(v) => setPhaseAll({ organic_spend: v })} min={0} step={100} />
        ...
        <AnimatedAccordion title="Conversion" defaultOpen>
          <div className="space-y-3">
            <TripleField label="Conv Trial (%)" help="% of paid installs that start a free trial. Good: 20–40%." values={[p1.conv_trial, p2.conv_trial, p3.conv_trial]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { conv_trial: v })} onChangeAll={(v) => setPhaseAll({ conv_trial: v })} min={0} max={100} step={1} slider />
            <TripleField label="Conv Paid (%)" help="% of trial users that convert to paid. Good: 10–25%." values={[p1.conv_paid, p2.conv_paid, p3.conv_paid]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { conv_paid: v })} onChangeAll={(v) => setPhaseAll({ conv_paid: v })} min={0} max={100} step={1} slider />
        ...
        <AnimatedAccordion title="Pricing & Revenue" defaultOpen>
          <div className="space-y-3">
            <TripleField label="Weekly ($)" help="Weekly subscription price" values={[p1.price_weekly, p2.price_weekly, p3.price_weekly]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { price_weekly: v })} onChangeAll={(v) => setPhaseAll({ price_weekly: v })} min={0} step={0.99} />
            <TripleField label="Monthly ($)" help="Monthly subscription price" values={[p1.price_monthly, p2.price_monthly, p3.price_monthly]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { price_monthly: v })} onChangeAll={(v) => setPhaseAll({ price_monthly: v })} min={0} step={0.99} />
            <TripleField label="Annual ($)" help="Annual subscription price" values={[p1.price_annual, p2.price_annual, p3.price_annual]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { price_annual: v })} onChangeAll={(v) => setPhaseAll({ price_annual: v })} min={0} step={0.99} />

            <div className="pt-1 text-[11px] font-medium text-muted-foreground">Plan Mix</div>
            <TripleField label="Weekly (%)" help="% choosing weekly plan. All should sum to 100%." values={[Math.round(p1.mix_weekly * 100), Math.round(p2.mix_weekly * 100), Math.round(p3.mix_weekly * 100)]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { mix_weekly: v / 100 })} onChangeAll={(v) => setPhaseAll({ mix_weekly: v / 100 })} min={0} max={100} step={1} slider />
            <TripleField label="Monthly (%)" help="% choosing monthly plan" values={[Math.round(p1.mix_monthly * 100), Math.round(p2.mix_monthly * 100), Math.round(p3.mix_monthly * 100)]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { mix_monthly: v / 100 })} onChangeAll={(v) => setPhaseAll({ mix_monthly: v / 100 })} min={0} max={100} step={1} slider />
            <TripleField label="Annual (%)" help="% choosing annual plan" values={[Math.round(p1.mix_annual * 100), Math.round(p2.mix_annual * 100), Math.round(p3.mix_annual * 100)]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { mix_annual: v / 100 })} onChangeAll={(v) => setPhaseAll({ mix_annual: v / 100 })} min={0} max={100} step={1} slider />
        ...
        <AnimatedAccordion title="Costs" defaultOpen>
          <div className="space-y-3">
            <TripleField label="Investment ($)" help="Initial investment / funding for this phase" values={[p1.investment, p2.investment, p3.investment]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { investment: v })} onChangeAll={(v) => setPhaseAll({ investment: v })} min={0} step={1000} />
            ...
            <TripleField label="Misc Costs ($)" help="Other monthly operational expenses" values={[p1.misc_total, p2.misc_total, p3.misc_total]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { misc_total: v })} onChangeAll={(v) => setPhaseAll({ misc_total: v })} min={0} step={100} />
            <TripleField label="COGS (%)" help="Cost of Goods Sold as % of revenue. SaaS/app: 15–30%." values={[Math.round(p1.cogs * 100), Math.round(p2.cogs * 100), Math.round(p3.cogs * 100)]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { cogs: v / 100 })} onChangeAll={(v) => setPhaseAll({ cogs: v / 100 })} min={0} max={100} step={1} />
          </div>
        </AnimatedAccordion>
        ...
        <AnimatedAccordion title="Advanced">
          <div className="space-y-3">
            <div className="text-[11px] font-medium text-muted-foreground">Retention & Churn</div>
            ...
            <div className="pt-2 text-[11px] font-medium text-muted-foreground">Taxes & Fees</div>
