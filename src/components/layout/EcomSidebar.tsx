      ...
      <div className="px-3 space-y-3 py-2">
        ...
        <AnimatedAccordion title="Acquisition" defaultOpen>
          <div className="space-y-3">
            ...
            <TripleField label="CPC ($)" help="Cost Per Click — average ad click price. Typical: $0.50–$3.00." values={[p1.cpc, p2.cpc, p3.cpc]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { cpc: v })} onChangeAll={(v) => setPhaseAll({ cpc: v })} min={0.01} step={0.1} />
        ...
        <AnimatedAccordion title="Conversion" defaultOpen>
          <div className="space-y-3">
            <TripleField label="Click-to-Purchase (%)" help="Conversion rate from ad click to purchase. Good: 2–5%." values={[p1.click_to_purchase, p2.click_to_purchase, p3.click_to_purchase]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { click_to_purchase: v })} onChangeAll={(v) => setPhaseAll({ click_to_purchase: v })} min={0} max={100} step={0.5} slider />
            <TripleField label="Repeat Purchase Rate (%)" help="% of customers who buy again within 30 days." values={[p1.repeat_purchase_rate, p2.repeat_purchase_rate, p3.repeat_purchase_rate]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { repeat_purchase_rate: v })} onChangeAll={(v) => setPhaseAll({ repeat_purchase_rate: v })} min={0} max={100} step={1} />
        ...
        <AnimatedAccordion title="Pricing & Revenue" defaultOpen>
          <div className="space-y-3">
            <TripleField label="AOV ($)" help="Average Order Value — mean revenue per order. E-commerce: $30–80." values={[p1.avg_order_value, p2.avg_order_value, p3.avg_order_value]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { avg_order_value: v })} onChangeAll={(v) => setPhaseAll({ avg_order_value: v })} min={0} step={1} />
            <TripleField label="Discount Rate (%)" help="Average discount across all orders. Reduces effective AOV." values={[p1.discount_rate, p2.discount_rate, p3.discount_rate]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { discount_rate: v })} onChangeAll={(v) => setPhaseAll({ discount_rate: v })} min={0} max={100} step={1} />
          </div>
        </AnimatedAccordion>
        ...
        <AnimatedAccordion title="Costs" defaultOpen>
          <div className="space-y-3">
            ...
            <TripleField label="COGS (%)" help="Cost of Goods Sold — manufacturing, shipping, packaging. Physical: 30–60%." values={[p1.cogs_pct, p2.cogs_pct, p3.cogs_pct]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { cogs_pct: v })} onChangeAll={(v) => setPhaseAll({ cogs_pct: v })} min={0} max={100} step={1} />
            <TripleField label="Return Rate (%)" help="% of orders returned or refunded. Fashion: 15–30%, electronics: 5–10%." values={[p1.return_rate, p2.return_rate, p3.return_rate]} perPhase={perPhase} onChange={(p, v) => setPhase(p, { return_rate: v })} onChangeAll={(v) => setPhaseAll({ return_rate: v })} min={0} max={100} step={0.5} />
